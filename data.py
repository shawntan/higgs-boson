...
#	data = (data - data.mean())/data.std()
	...
	data['met_x'],data['met_y'],_ = momentum_vec(data['PRI_met'],data['PRI_met_phi'])
	"""
	"""
	for col in columns:
...
#		data["jet_%d"%i][data.PRI_jet_num == i] = 1
	normalise_mask = np.ones(len(data.columns),dtype=np.bool)
	normalise_mask[[data.columns.get_loc(i) for i in columns]] = False
	return data.values,labels,weights,normalise_mask,df['EventId'],[ n for n in data ]
